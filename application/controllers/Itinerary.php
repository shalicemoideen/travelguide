...
// }
	...
	private function _upload_day_image($fieldName, $index)
	{
		...
		// $config['encrypt_name']  = true;
		// // ✅ you said: no file size validation => DON'T set max_size

		// /* IMPORTANT */
		// $config['detect_mime']   = FALSE;   // disable strict mime checking
		// print_r($config);die;
// 		echo '<pre>';
  ...
  public function ajax_add()
{
    ...
    if ($insert) {
        ...
        //     // ✅ upload image for this row index
        //     $img = $this->_upload_day_image('itineraries_days_image', $k);

        //     // if upload error, return now for debugging
        //     if (strpos($img, '__ERROR__:') === 0) {
        //         echo json_encode(array("status"=>FALSE, "message"=>$img));
        //         return;
        //     }
        ...
        //         'itineraries_days_image'             => $img,
        //         'itineraries_days_status'            => 1
        //     );

        //     $this->db->insert('itineraries_days', $row);
        // }
		...
		foreach ($days_day as $k => $dayNo) {
			...
			$titleVal = isset($days_title[$k]) ? $days_title[$k] : '';
			$descVal  = isset($days_desc[$k]) ? $days_desc[$k] : '';
			$tbVal    = isset($days_travel_back[$k]) ? $days_travel_back[$k] : '';
