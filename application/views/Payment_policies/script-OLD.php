...
//            $('td',row).eq(0).html(index+1);
           $table.column(0).nodes().each(function(node,index,dt){
            ...
            // Edit button
            if (hasPermission('PAYMENT_POLICY_UPDATE')) {
                actionHtml += '<a href="javascript:void(0)" onclick="edit_payment_policies('+data['payment_policies_id']+')" class="btn btn-primary shadow btn-xs sharp me-1"><i class="fas fa-pencil-alt"></i></a>';
            }

            // Delete button
            if (hasPermission('PAYMENT_POLICY_DELETE')) {
                actionHtml += '<a href="javascript:void(0)" onclick="return delete_payment_policies('+data['payment_policies_id']+')" class="btn btn-danger shadow btn-xs sharp"><i class="fa fa-trash"></i></a>';
            }
