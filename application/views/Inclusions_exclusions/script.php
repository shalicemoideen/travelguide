...
//            $('td',row).eq(0).html(index+1);
           ...
           let actionHtml = '<div class="d-flex">';
            ...
            // Edit button
            if (hasPermission('INCLUSION_AND_EXCLUSION_UPDATE')) {
                actionHtml += '<a href="javascript:void(0)" onclick="edit_inclusion_exclusion('+data['inclusion_exclusion_common_id']+')" class="btn btn-primary shadow btn-xs sharp me-1"><i class="fas fa-pencil-alt"></i></a>';
            }

            // Delete button
            if (hasPermission('INCLUSION_AND_EXCLUSION_DELETE')) {
                actionHtml += '<a href="javascript:void(0)" onclick="return delete_inclusion_exclusion('+data['inclusion_exclusion_common_id']+')" class="btn btn-danger shadow btn-xs sharp"><i class="fa fa-trash"></i></a>';
            }
...
function save()
{
    ...
    // Title validation
    let title = $.trim($('#inclusion_exclusion_common_title').val());
    if (title === '') {
        $('#inclusion_exclusion_common_title').closest('.form-group').addClass('input-warning-o');
        $('#inclusion_exclusion_common_title').next('.help-block').text('Title is required');
        isValid = false;
    }

    // Inclusions validation
    $('textarea[name^="inclusions_details"]').each(function () {
        let value = $.trim($(this).val());
        if (value === '') {
            $(this).parent().addClass('input-warning-o');
            $(this).next('.help-block').text('Inclusion is required');
            isValid = false;
        }
    });

    // Exclusions validation
    $('textarea[name^="exclusions_details"]').each(function () {
        let value = $.trim($(this).val());
        if (value === '') {
            $(this).parent().addClass('input-warning-o');
            $(this).next('.help-block').text('Exclusion is required');
            isValid = false;
        }
    });
    ...
    $.ajax({
        ...
        success: function(data)
        {
            if (data.status) {
                ...
                for (var i = 0; i < data.inputerror.length; i++) {
                    var field = $('[name="'+data.inputerror[i]+'"]');
                    field.parent().addClass('input-warning-o');
                    field.next('.help-block').text(data.error_string[i]);
                }
            }
