...
//            $('td',row).eq(0).html(index+1);
           ...
           let actionHtml = '<div class="d-flex">';
            ...
            if (hasPermission('VEHICLE_UPDATE')) {
                actionHtml += '<a href="javascript:void(0)" onclick="edit_vehicle('+data['vehicle_id']+')" class="btn btn-primary shadow btn-xs sharp me-1"><i class="fas fa-pencil-alt"></i></a>';
            }

            // Delete button
            if (hasPermission('VEHICLE_DELETE')) {
                actionHtml += '<a href="javascript:void(0)" onclick="return delete_vehicle('+data['vehicle_id']+')" class="btn btn-danger shadow btn-xs sharp"><i class="fa fa-trash"></i></a>';
            }
...
function Vehiclemodalclose()
{
	...
	$('#vehicle_name_alert').hide();
	$('.submit').removeAttr('disabled');
	$('.form-group').removeClass('input-success-o');
	$('.form-group').removeClass('input-warning-o');
	$('.vehicle_name').removeClass('input-success-o');
	$('.vehicle_name').removeClass('input-warning-o');
    $('.vehicle_description').removeClass('input-success-o');
    $('.vehicle_description').removeClass('input-warning-o');
...
////***For open modal of Vehicle adding form  *****///
...
{ 
    ...
    $('#VehicleModal').modal('show'); // show bootstrap modal
    $('.modal-title').text('Add Vehicle Details'); // Set Title to Bootstrap modal title
	$('#btnSave').text('save');
}

////***For open modal of stage adding form  *****///

////***For editing vehicle details from adding modal form  *****///
...
function edit_vehicle(id)
{
    ...
    $.ajax({
        url : "<?php echo base_url();?>index.php/Vehicle/ajax_edit/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {

            $('[name="id"]').val(data.vehicle_id);
            $('[name="vehicle_name"]').val(data.vehicle_name);
            $('[name="vehicle_number_seat"]').val(data.vehicle_number_seat);
            $('[name="vehicle_description"]').val(data.vehicle_description);      
            $('#VehicleModal').modal('show'); // show bootstrap modal when complete loaded
            $('.modal-title').text('Edit Vehicle Details'); // Set title to Bootstrap modal title
			$('#btnSave').text('update');
        },
...
function reload_table()
{
	...
	{  
		...
		ff = "Vehicle details updated successfully";
		...
		ff = "Vehicle details added successfully";
...
///***For save the vehicle details from adding modal form *****///
...
function save()
{
    ...
    if(save_method == 'add') {
        ...
        url = "<?php echo base_url();?>index.php/Vehicle/ajax_add/";
    } 
	else {
        ...
        url = "<?php echo base_url();?>index.php/Vehicle/ajax_update/";
    }
...
function reload_table_delete()
{
    ...
    // swal("Vehicle details deleted successfully", "", "success")
        ...
        ff = "Vehicle details deleted successfully";
...
////***For reload the vehicle datatable for delete *****///
...
function delete_vehicle_action()
{
    ...
    var url;
        ...
        url = "<?php echo base_url();?>index.php/Vehicle/delete/";
...
////***For delete the vehicle details from  database *****///

////***Checking vehicle name already existing*****///

$("#vehicle_name").on("input", function(e) {
...
if(vehicle_name){
    $.ajax({
        url:"<?php echo base_url()?>index.php/Vehicle/checkEditvehicle",
        type: 'POST',
        data: {value:vehicle_name,id:id},
        dataType: 'json',
        success:
        function(data)
        {
            //alert(data);
            if(data){
                var data1 = "Vehicle Already Exist";
                $("#vehicle_name_alert").html(data1);
                $("#vehicle_name_alert").show();
                $('#vehicle_name').val('');
                $('#vehicle_name').focus();
                $('#btnSave').attr("disabled", "disabled");
                $('.vehicle_name').removeClass('input-success-o').addClass('input-warning-o');
             }
             else{
                 $('#btnSave').removeAttr('disabled');
                  $("#vehicle_name_alert").hide();
                  $('.vehicle_name').removeClass('input-warning-o').addClass('input-success-o');
             }
 ...
 if(vehicle_name){
    $.ajax({
        url:"<?php echo base_url()?>index.php/Vehicle/checkvehicle",
        type: 'POST',
        data: {value:vehicle_name},
        dataType: 'json',
        success:
        function(data)
        {
