...
//            $('td',row).eq(0).html(index+1);
           $table.column(0).nodes().each(function(node,index,dt){
            ...
            // Edit button
            if (hasPermission('PROPERTY_CATEGORY_UPDATE')) {
                actionHtml += '<a href="javascript:void(0)" onclick="edit_category('+data['property_category_id']+')" class="btn btn-primary shadow btn-xs sharp me-1"><i class="fas fa-pencil-alt"></i></a>';
            }

            // Delete button
            if (hasPermission('PROPERTY_CATEGORY_DELETE')) {
                actionHtml += '<a href="javascript:void(0)" onclick="return delete_category('+data['property_category_id']+')" class="btn btn-danger shadow btn-xs sharp"><i class="fa fa-trash"></i></a>';
            }
...
////***For close the modal *****///
function propertycategorymodalclose()
{

    $('#PropertycategoryModal').modal('hide');
   
	//$( "div" ).remove( ".modal-backdrop" );
    $('#property_category_name').val('');
    $('#property_category_description').val('');
    ...
    $('.property_category_description').removeClass('input-success-o');
    $('.property_category_description').removeClass('input-warning-o');
	// $('#btnSave').removeAttr('disabled');
}
////***For close the modal *****///

////***For open the modal *****///
$('#PropertycategoryModal').on('shown.bs.modal', function () {
    // $("#state_id_fk").select2('open');
	$('#property_category_name').focus();
    $('#category_name_alert').hide();
...
////***For open modal of Property category adding form  *****///
...
{ 
    ...
    $('#PropertycategoryModal').modal('show'); // show bootstrap modal
    $('.modal-title').text('Add Property category Details'); // Set Title to Bootstrap modal title
	$('#btnSave').text('save');
}

////***For open modal of stage adding form  *****///

////***For editing Property category details from adding modal form  *****///
...
function edit_category(id)
{
    ...
    $.ajax({
        url : "<?php echo base_url();?>index.php/Property_category/ajax_edit/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {

            $('[name="id"]').val(data.property_category_id);
            $('[name="property_category_name"]').val(data.property_category_name);
            $('[name="property_category_description"]').val(data.property_category_description);      
            $('#PropertycategoryModal').modal('show'); // show bootstrap modal when complete loaded
            $('.modal-title').text('Edit Property category Details'); // Set title to Bootstrap modal title
			$('#btnSave').text('update');
        },
...
function reload_table()
{
	...
	{  
		...
		ff = "Property category details updated successfully";
		...
		ff = "Property category details added successfully";
...
///***For save the Property category details from adding modal form *****///
...
function save()
{
    ...
    if(save_method == 'add') {
        ...
        url = "<?php echo base_url();?>index.php/Property_category/ajax_add/";
    } 
	else {
        ...
        url = "<?php echo base_url();?>index.php/Property_category/ajax_update/";
    }
...
function reload_table_delete()
{
    ...
    // swal("Property category details deleted successfully", "", "success")
        ...
        ff = "Property category details deleted successfully";
...
function delete_category_action()
{
    ...
    var url;
        ...
        url = "<?php echo base_url();?>index.php/Property_category/delete/";
...
////***For delete the Property category details from  database *****///

////***Checking Property category name already existing*****///

// $("#property_category_name").on("input", function(e) {

// var property_category_name =$("#property_category_name").val();
// var id = $ ("#id").val();
// //alert(product_id);
// if(id){
// if(property_category_name){
//     $.ajax({
//         url:"<?php echo base_url()?>index.php/Property_category/checkEditproperty_category",
//         type: 'POST',
//         data: {value:property_category_name,id:id},
//         dataType: 'json',
//         success:
//         function(data)
//         {
//             //alert(data);
//             if(data){
//                 var data1 = "Property category Already Exist";
//                 $("#category_name_alert").html(data1);
//                 $("#category_name_alert").show();
//                 $('#vehicle_name').val('');
//                 $('#vehicle_name').focus();
//                 $('#btnSave').attr("disabled", "disabled");
//                 $('.property_category_name').removeClass('input-success-o').addClass('input-warning-o');
//              }
//              else{
//                  $('#btnSave').removeAttr('disabled');
//                   $("#category_name_alert").hide();
//                   $('.property_category_name').removeClass('input-warning-o').addClass('input-success-o');
//              }
...
//  if(property_category_name){
//     $.ajax({
//         url:"<?php echo base_url()?>index.php/Property_category/checkproperty_category",
//         type: 'POST',
//         data: {value:property_category_name},
//         dataType: 'json',
//         success:
//         function(data)
//         {
