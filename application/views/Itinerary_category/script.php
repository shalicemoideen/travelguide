...
//            $('td',row).eq(0).html(index+1);
           $table.column(0).nodes().each(function(node,index,dt){
			...
			// $('td', row).eq(4).html('<div class="d-flex"><a href="javascript:void(0)" onclick="edit_category('+data['itinerary_category_id']+')" class="btn btn-primary shadow btn-xs sharp me-1"><i class="fas fa-pencil-alt"></i></a><a href="javascript:void(0)" onclick="return delete_category('+data['itinerary_category_id']+')" class="btn btn-danger shadow btn-xs sharp"><i class="fa fa-trash"></i></a></div>');
			
            let actionHtml = '<div class="d-flex">';

            // Edit button
            if (hasPermission('CATEGORY_UPDATE')) {
                actionHtml += '<a href="javascript:void(0)" onclick="edit_category('+data['itinerary_category_id']+')" class="btn btn-primary shadow btn-xs sharp me-1"><i class="fas fa-pencil-alt"></i></a>';
            }

            // Delete button
            if (hasPermission('CATEGORY_DELETE')) {
                actionHtml += '<a href="javascript:void(0)" onclick="return delete_category('+data['itinerary_category_id']+')" class="btn btn-danger shadow btn-xs sharp"><i class="fa fa-trash"></i></a>';
            }
...
////***For close the modal *****///
function itinerarycategorymodalclose()
{

    $('#ItinerarycategoryModal').modal('hide');
   
	//$( "div" ).remove( ".modal-backdrop" );
    $('#itinerary_category_name').val('');
    $('#itinerary_category_description').val('');
    ...
    $('.itinerary_category_description').removeClass('input-success-o');
    $('.itinerary_category_description').removeClass('input-warning-o');
	// $('#btnSave').removeAttr('disabled');
}
////***For close the modal *****///

////***For open the modal *****///
$('#ItinerarycategoryModal').on('shown.bs.modal', function () {
...
////***For open modal of Itinerary category adding form  *****///
...
{ 
    ...
    $('#ItinerarycategoryModal').modal('show'); // show bootstrap modal
    $('.modal-title').text('Add Itinerary category Details'); // Set Title to Bootstrap modal title
	$('#btnSave').text('save');
}

////***For open modal of stage adding form  *****///

////***For editing Itinerary category details from adding modal form  *****///
...
function edit_category(id)
{
    ...
    $.ajax({
        url : "<?php echo base_url();?>index.php/Itinerary_category/ajax_edit/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {

            $('[name="id"]').val(data.itinerary_category_id);
            $('[name="itinerary_category_name"]').val(data.itinerary_category_name);
            $('[name="itinerary_category_description"]').val(data.itinerary_category_description);      
            $('#ItinerarycategoryModal').modal('show'); // show bootstrap modal when complete loaded
            $('.modal-title').text('Edit Itinerary category Details'); // Set title to Bootstrap modal title
			$('#btnSave').text('update');
        },
...
function reload_table()
{
	...
	{  
        ...
        // swal("Itinerary category details updated successfully", "", "success")
		var ff = 0;
		
		ff = "Itinerary category details updated successfully";
        ...
        // swal("Itinerary category details added successfully", "", "success")

		var ff = 0;
		
		ff = "Itinerary category details added successfully";
...
///***For save the Itinerary category details from adding modal form *****///
...
function save()
{
    ...
    if(save_method == 'add') {
        ...
        url = "<?php echo base_url();?>index.php/Itinerary_category/ajax_add/";
    } 
	else {
        ...
        url = "<?php echo base_url();?>index.php/Itinerary_category/ajax_update/";
    }
...
function reload_table_delete()
{
    ...
    // swal("Itinerary category details deleted successfully", "", "success")
        ...
        ff = "Itinerary category details deleted successfully";
...
////***For reload the Itinerary category datatable for delete *****///
...
function delete_category_action()
{
    ...
    var url;
        ...
        url = "<?php echo base_url();?>index.php/Itinerary_category/delete/";
...
////***For delete the Itinerary category details from  database *****///

////***Checking Itinerary category name already existing*****///

$("#itinerary_category_name").on("input", function(e) {

var itinerary_category_name =$("#itinerary_category_name").val();
var id = $ ("#id").val();
//alert(product_id);
if(id){
if(itinerary_category_name){
    $.ajax({
        url:"<?php echo base_url()?>index.php/Itinerary_category/checkEdititinerary_category",
        type: 'POST',
        data: {value:itinerary_category_name,id:id},
        dataType: 'json',
        success:
        function(data)
        {
            //alert(data);
            if(data){
                var data1 = "Itinerary category Already Exist";
                $("#category_name_alert").html(data1);
                $("#category_name_alert").show();
                $('#itinerary_category_name').val('');
                $('#itinerary_category_name').focus();
                $('#btnSave').attr("disabled", "disabled");
                $('.itinerary_category_name').removeClass('input-success-o').addClass('input-warning-o');
             }
             else{
                 $('#btnSave').removeAttr('disabled');
                  $("#category_name_alert").hide();
                  $('.itinerary_category_name').removeClass('input-warning-o').addClass('input-success-o');
             }
 ...
 if(itinerary_category_name){
    $.ajax({
        url:"<?php echo base_url()?>index.php/Itinerary_category/checkitinerary_category",
        type: 'POST',
        data: {value:itinerary_category_name},
        dataType: 'json',
        success:
        function(data)
        {
