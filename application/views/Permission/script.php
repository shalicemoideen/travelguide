...
var table;
  $(document).ready(function() {
    ...
    $table = $('#Permission_table').DataTable( {
        ...
        "ajax": {
            "url": "<?php echo base_url();?>index.php/Permission/get/",
            "type": "POST",
            "data" : function (d) {
        ...
        "createdRow": function ( row, data, index ) {
			...
			$('td', row).eq(4).html('<div class="d-flex"><a href="javascript:void(0)" onclick="edit_permission('+data['id']+')" class="btn btn-primary shadow btn-xs sharp me-1"><i class="fas fa-pencil-alt"></i></a><a href="javascript:void(0)" onclick="return delete_permission('+data['id']+')" class="btn btn-danger shadow btn-xs sharp"><i class="fa fa-trash"></i></a></div>');
...
{ 
    ...
    $('#PermissionModal').modal('show'); // show bootstrap modal
    $('.modal-title').text('Add Permission Details'); // Set Title to Bootstrap modal title
	$('#btnSave').text('save');
}
...
function edit_permission(id)
{
    ...
    $.ajax({
        url : "<?php echo base_url();?>index.php/Permission/ajax_edit/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
            ...
            $('[name="id"]').val(data.id);
            $('[name="permission_name"]').val(data.name);
            $('[name="permission_description"]').val(data.description);
            $('[name="permission_status"]').val(data.status);      
            $('#PermissionModal').modal('show'); // show bootstrap modal when complete loaded
            $('.modal-title').text('Edit Permission Details'); // Set title to Bootstrap modal title
			$('#btnSave').text('update');
        },
...
function reload_table()
{
	...
	{  
		...
		ff = "Permission details updated successfully";
		...
		ff = "Permission details added successfully";
...
function save()
{
    ...
    if(save_method == 'add') {
        ...
        url = "<?php echo base_url();?>index.php/Permission/ajax_add/";
    } 
	else {
        ...
        url = "<?php echo base_url();?>index.php/Permission/ajax_update/";
    }
...
function reload_table_delete()
{
    ...
    // swal("Package category details deleted successfully", "", "success")
        ...
        ff = "Permission details deleted successfully";
...
function delete_permission_action()
{
    ...
    var url;
        ...
        url = "<?php echo base_url();?>index.php/Permission/delete/";
