...
function createStandardContinuationPage(fontClass) {

  const page = document.createElement('div');

  page.className = 'pdf-page property-page standard-cont-page generated-option-page';

  if (fontClass) page.classList.add(fontClass);
...
function createExclusiveContinuationPage(fontClass) {
...
function fitOptionPageFontSize(page, type) {
  ...
  var targetHeight = page.clientHeight;

  var origMinHeight = content.style.minHeight;

  var origOverflow = content.style.overflow;

  var origHeight = content.style.height;

  var origPosition = content.style.position;

  var origVisibility = content.style.visibility;

  var origWidth = content.style.width;

  var origZIndex = content.style.zIndex;



  // Move content out of flow so it grows to natural height regardless of page's fixed height.

  // This is the key fix for Safari — scrollHeight on a child of a fixed-height container

  // does not report the true content height, but offsetHeight on an absolutely-positioned

  // auto-height element does.
  ...
  var tryOrder = ['font-xl', 'font-lg', 'font-normal', 'font-sm', 'font-xs'];

  var chosenClass = 'font-xs';
  ...
  for (var i = 0; i < tryOrder.length; i++) {
    ...
    if (content.offsetHeight <= targetHeight + 2) {
  ...
  content.style.minHeight = origMinHeight;

  content.style.height = origHeight;

  content.style.overflow = origOverflow;

  content.style.position = origPosition;

  content.style.visibility = origVisibility;

  content.style.width = origWidth;

  content.style.zIndex = origZIndex;
...
function splitStandardOptionPage(page) {
  ...
  var fontClasses = ['font-xs', 'font-sm', 'font-normal', 'font-lg', 'font-xl'];

  var pageFontClass = '';

  fontClasses.forEach(function(fc) { if (page.classList.contains(fc)) pageFontClass = fc; });

  let contPage = createStandardContinuationPage(pageFontClass);
  ...
  var contentLimit = content.getBoundingClientRect().top + content.clientHeight;
  ...
  overflowItems.forEach(function(item) {
    ...
    if (item.type === 'row') {
      ...
      if (item.node.getBoundingClientRect().bottom > contentLimit) {

        currentTbody.removeChild(item.node);



        contPage = createStandardContinuationPage(pageFontClass);
        ...
        contentLimit = content.getBoundingClientRect().top + content.clientHeight;
...
function splitExclusiveOptionPage(page) {
  ...
  let contPage = createExclusiveContinuationPage(pageFontClass);
  ...
  overflowItems.forEach(function(item) {
    ...
    if (item.type === 'row') {
      ...
      if (item.node.getBoundingClientRect().bottom > contentLimit) {
        ...
        contPage = createExclusiveContinuationPage(pageFontClass);
...
function fitBriefPageFontSize(page) {
  ...
  var origListMinHeight = list.style.minHeight;

  var origListOverflow = list.style.overflow;

  var origListHeight = list.style.height;

  var origListPosition = list.style.position;

  var origListVisibility = list.style.visibility;

  var origListWidth = list.style.width;

  var origListZIndex = list.style.zIndex;

  var origHeaderOverflow = header.style.overflow;



  // Move list out of flow so it grows to natural height (Safari fix).
  ...
  list.style.width = page.clientWidth + 'px';
  ...
  var tryOrder = ['font-normal', 'font-sm', 'font-xs'];
  ...
  for (var i = 0; i < tryOrder.length; i++) {
    ...
    if (header.offsetHeight + list.offsetHeight <= targetHeight + 2) {
  ...
  list.style.minHeight = origListMinHeight;

  list.style.height = origListHeight;

  list.style.overflow = origListOverflow;

  list.style.position = origListPosition;

  list.style.visibility = origListVisibility;

  list.style.width = origListWidth;

  list.style.zIndex = origListZIndex;

  header.style.overflow = origHeaderOverflow;
